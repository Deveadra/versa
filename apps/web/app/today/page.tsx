...
import { fetchGoals, fetchPlanner, fetchSchedule, fetchStudyAssignments, fetchTasks } from '../../lib/api';
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        setData({ planner: planner.data, todayTasks: tasks.data, overdue: overdue.data, schedule: schedule.data, goals: goals.data, study: study.data });
      } catch (err) {
  ...
  return (
    <section>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(240px,1fr))', gap: 16 }}>
        <article style={{ border: '1px solid #334155', padding: 12 }}>
          ...
          {Array.isArray(planner.priorities) && (planner.priorities as string[]).length > 0 ? (
            <ul>{(planner.priorities as string[]).map((item) => <li key={item}>{item}</li>)}</ul>
          ) : <p>No priorities yet.</p>}
        </article>

        <article style={{ border: '1px solid #334155', padding: 12 }}>
          <h3>Upcoming schedule</h3>
          {schedule.length ? <ul>{schedule.slice(0, 4).map((block) => <li key={block.id}>{block.start_time}-{block.end_time} {block.title}</li>)}</ul> : <p>No blocks planned.</p>}
        </article>

        <article style={{ border: '1px solid #334155', padding: 12 }}>
          <h3>Study block</h3>
          <p>{study.filter((assignment) => assignment.status !== 'done').length} assignments pending</p>
        </article>
      ...
      <div style={{ marginTop: 20, display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(240px,1fr))', gap: 16 }}>
        <article style={{ border: '1px solid #334155', padding: 12 }}>
          <h3>Overdue items</h3>
          {overdue.length ? <ul>{overdue.map((task) => <li key={task.id}>{task.title}</li>)}</ul> : <p>No overdue tasks.</p>}
        </article>
        ...
        <article style={{ border: '1px solid #334155', padding: 12 }}>
          ...
          <ul>
            ...
            <li><Link href="/goals">+ Goal</Link></li>
            <li><Link href="/schedule">+ Schedule block</Link></li>
            <li><Link href="/jobs">+ Job lead</Link></li>
          </ul>
        </article>

        <article style={{ border: '1px solid #334155', padding: 12 }}>
          <h3>Today tasks</h3>
          {todayTasks.length ? <ul>{todayTasks.map((task) => <li key={task.id}>{task.title}</li>)}</ul> : <p>No tasks scheduled for today.</p>}
        </article>
      </div>
    </section>
  );
