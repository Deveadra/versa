...
def rrule_from_phrase(phrase: str, dtstart_iso: str | None = None) -> str | None:
    ...
    # every <weekday> at X
    m = re.search(r"every (monday|tuesday|wednesday|thursday|friday|saturday|sunday) at ([0-9:apm]+)", p)
    if m:
    ...
    m = re.search(r"weekly on (monday|tuesday|wednesday|thursday|friday|saturday|sunday) at ([0-9:apm]+)", p)
    if m:
        byday = WEEKDAYS[m.group(1)]
        h, mi, s = _time_to_hms(m.group(2))
        return f"FREQ=WEEKLY;BYDAY={byday};BYHOUR={h};BYMINUTE={mi};BYSECOND={s}"
