...
class CalendarStore:
    ...
    def list_events(self) -> list[dict]:
        cur = self.db.conn.execute(
            "SELECT id, title, start, end, rrule, location, attendees, meta FROM calendar_events"
        )
        rows = []
    ...
    def expand(self, window_start_iso: str, window_end_iso: str, limit: int = 200) -> list[dict]:
        ...
        for e in self.list_events():
            base_start = datetime.fromisoformat(e["start"].replace("Z", "+00:00")).astimezone(
                tz.UTC
            )
            base_end = (
                datetime.fromisoformat(e["end"].replace("Z", "+00:00")).astimezone(tz.UTC)
                if e["end"]
                else None
            )
            ...
            for dt in rule.between(wstart - timedelta(days=1), wend + timedelta(days=1), inc=True):
                ...
                inst_end = (
                    inst_start + ((base_end - base_start) if base_end else timedelta(hours=1))
                ).astimezone(tz.UTC)
