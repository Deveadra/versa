...
from base.apps.file_manager_prompts import (
...
def has_pending():
    return (
        any(
            v is None for k, v in file_state.items() if k not in ["confirm", "candidates", "chosen"]
        )
        or file_state["confirm"]
        or (file_state["candidates"] and not file_state["chosen"])
    )
    ...
    if file_state["candidates"] and not file_state["chosen"]:
        try:
            ...
            if 0 <= idx < len(file_state["candidates"]):
                ...
                return (
                    None,
                    f"You selected {file_state['chosen']}. Do you want me to {file_state['action']} it?",
                )
            else:
        ...
        if text.lower() in ["yes", "confirm", "do it"]:
            ...
            if file_state["action"] == "search":
                ...
                if not matches:
                    ...
                    path = matches[0]
                    result = (
                        open_file(path) if file_state["action"] == "open" else summarize_file(path)
                    )
                else:
                    options = " | ".join(
