...
def record_feedback(conn: sqlite3.Connection, rule_id: int, topic_id: str, tone: str, context: str, outcome: str):
    """
    ...
    row = conn.execute("SELECT ema_success, ema_negative FROM rule_stats WHERE rule_id=?", (rule_id,)).fetchone()
    if not row:
        conn.execute("""
            INSERT INTO rule_stats(rule_id, last_fired, fires_today, ema_success, ema_negative)
            VALUES(?, datetime('now'), 0, ?, ?)
        """, (rule_id, 0.5 if outcome in ("acted","thanks") else 0.0,
        ...
        conn.execute("UPDATE rule_stats SET ema_success=?, ema_negative=? WHERE rule_id=?", (suc, neg, rule_id))
    
    conn.commit()

def schedule_signal_check(conn, rule_id, topic_id, tone, context, signal_names, expect_change, delay=300):
    """
