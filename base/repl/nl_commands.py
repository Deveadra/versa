...
def handle_nl_command(cmd: str) -> str:
    ...
    # MEMORY
    # -------------------
    if any(kw in cmd_low for kw in ["do you remember", "what did i tell you", "did i mention", "have you stored"]):
    ...
    # KNOWLEDGE GRAPH
    # -------------------
    if any(phrase in cmd_low for phrase in ["used to", "was my", "were my", "formerly", "in the past"]):
        return orch.query_kg_context(cmd)

    if re.search(r"(19|20)\d{2}", cmd_low) or any(kw in cmd_low for kw in [
        "last week", "next week", "last month", "next month", "yesterday", "tomorrow",
        "two weeks ago", "days ago", "next year", "last year"
    ]):
        return orch.query_kg_context(cmd)

    if any(kw in cmd_low for kw in [
        "who", "where", "relation", "related", "boss", "parent", "child", "husband",
        "wife", "job", "work", "friend", "teacher", "attends", "sleep"
    ]):
        return orch.query_kg_context(cmd)
    ...
    # CALENDAR / EVENTS
    # -------------------
    if any(x in cmd_low for x in [
        "what's on my calendar", "my meetings", "my schedule", "agenda", "appointments", "events", "upcoming"
    ]):
        # detect window
        days = 14
    ...
    if any(kw in cmd_low for kw in [
        "who", "where", "relation", "related", "boss", "parent", "child", "husband", "wife",
        "job", "work", "sleep", "friend", "teacher", "attends"
    ]):
        return orch.query_kg_context(cmd)
    ...
    if cmd_low.startswith("add") or cmd_low.startswith("schedule"):
        ...
        title_match = re.match(r"(?:add|schedule) (?:a |an )?(.+?) (every|weekly|daily|monthly)", cmd_low)
        if title_match:
    ...
    # --- Upcoming events ---
    if any(x in cmd_low for x in ["what's on my calendar", "upcoming", "next week", "next month", "my schedule"]):
        days = 14
        if "next week" in cmd_low:
            days = 7
        elif "next month" in cmd_low:
