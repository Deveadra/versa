...
from base.llm.prompt_composer import compose_persona_block, compose_prompt, compose_retrieval_block, justify_memory
...
class PromptComposerTests(unittest.TestCase):
    ...
    def test_justify_memory_and_retrieval_block(self):
        mems = [
            {"summary": "Sundance likes lo-fi on Spotify", "source": "usage_log", "score": 0.92, "last_used": "2025-09-10T12:00:00"},
            {"summary": "Prefers short replies in mornings", "source": "usage_log", "score": 0.6, "last_used": "2025-09-18T08:00:00"},
    ...
    def test_compose_prompt_full(self):
        sys = "You are Ultron. Be concise and warm."
        persona = "Name: Sundance\npreferred_player: Spotify\nfavorite_music: lo-fi\n"
        mems = [{"summary": "Sundance likes lo-fi on Spotify", "source": "usage_log", "score": 0.92, "last_used": "2025-09-10T12:00:00"}]
        prompt = compose_prompt(sys, "Play my usual", persona_text=persona, memories=mems, extra_context="KG: none", top_k_memories=1)
...
res = subprocess.run([sys.executable, "-m", "unittest", "discover", "-v", str(tests_dir)], cwd=str(root), capture_output=True, text=True)
